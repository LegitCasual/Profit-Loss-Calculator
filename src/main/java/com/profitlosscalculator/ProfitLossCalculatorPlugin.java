...
public class ProfitLossCalculatorPlugin extends Plugin implements ProfitLossCalculatorPanel.Controls
{
	...
	@Inject
	private ProfitLossCalculatorConfig config;
	...
	private ProfitLossCalculatorPanel.View frozenView;
	...
	@Provides
	ProfitLossCalculatorConfig provideConfig(ConfigManager configManager)
	{
		return configManager.getConfig(ProfitLossCalculatorConfig.class);
	}

	/** Config group before the plugin was renamed from "Session Cost Tracker". */
	private static final String LEGACY_CONFIG_GROUP = "sessioncosttracker";

	/**
	 * One-time move of saved settings from the pre-rename config group to
	 * {@link ProfitLossCalculatorConfig#GROUP}. Without this every user's settings would
	 * silently reset when they update. Runs once, then leaves a marker so it never repeats.
	 */
	private void migrateSettings()
	{
		final String group = ProfitLossCalculatorConfig.GROUP;
		if (configManager.getConfiguration(group, "migratedFrom") != null)
		{
			return;
		}
		final String prefix = LEGACY_CONFIG_GROUP + ".";
		for (String wholeKey : configManager.getConfigurationKeys(prefix))
		{
			final String key = wholeKey.substring(prefix.length());
			final String value = configManager.getConfiguration(LEGACY_CONFIG_GROUP, key);
		...
		configManager.setConfiguration(group, "migratedFrom", LEGACY_CONFIG_GROUP);
	}
	...
	{
		migrateSettings();
		panel = new ProfitLossCalculatorPanel(this, itemManager, !config.welcomeSeen());

		BufferedImage icon = ImageUtil.loadImageResource(getClass(), "panel_icon.png");
		navButton = NavigationButton.builder()
			.tooltip("Profit Loss Calculator")
			.icon(icon)
			.priority(7)
			.panel(panel)
			.build();
	...
	{
		final ProfitLossCalculatorPanel.View view = buildView();
	...
	private ProfitLossCalculatorPanel.View buildView()
	{
		...
		for (DeathEntry e : session.getDeaths())
		{
			...
			b.death(new ProfitLossCalculatorPanel.DeathRow(e.getId(),
	...
	private List<ProfitLossCalculatorPanel.GridItem> gainGridItems(Set<IncomeEvent> only)
	{
		final boolean full = config.countUncollectedDrops();
	...
	private List<ProfitLossCalculatorPanel.GridItem> gridItems(Map<Integer, Integer> qty, ItemPricer pricer)
	{
		...
		qty.forEach((id, q) ->
		{
			if (q > 0)
			{
				out.add(new ProfitLossCalculatorPanel.GridItem(
					id, q, pricer.value(id, q), itemName(id)));
			}
		});
		out.sort(Comparator.comparingLong(ProfitLossCalculatorPanel.GridItem::getValue).reversed());
		return out;
	}
	 ...
	 */
	private List<ProfitLossCalculatorPanel.EventLine> incomeLines(Session s, Set<IncomeEvent> inKill)
	{
		final long floor = Math.max(0, config.ignoreIncomeBelow());
		...
		final List<ProfitLossCalculatorPanel.EventLine> lines = new ArrayList<>();
		agg.forEach((src, a) ->
		{
			if (a[1] < floor)
			{
			...
			tip.append("</html>");
			lines.add(new ProfitLossCalculatorPanel.EventLine(kind.get(src), "", src, a[0], tip.toString()));
		});
		lines.sort(Comparator.comparingLong(ProfitLossCalculatorPanel.EventLine::getGp).reversed());
		return lines;
	}
	...
	private List<ProfitLossCalculatorPanel.EventLine> eventLines(Session s)
	{
		final List<ProfitLossCalculatorPanel.EventLine> lines = new ArrayList<>();
		...
		lines.sort(Comparator.comparing(ProfitLossCalculatorPanel.EventLine::getTime));
