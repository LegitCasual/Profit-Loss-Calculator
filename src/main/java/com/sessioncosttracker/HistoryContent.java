...
class HistoryContent extends JPanel
{
	...
	HistoryContent(SessionCostTrackerPanel.Controls controls, ItemManager itemManager)
	{
	...
	private JPanel mobBox(SessionHistory.MobStats m)
	{
		...
		try
		{
			final List<SessionCostTrackerPanel.GridItem> items = gridItems(m.getItems());
			if (!items.isEmpty())
			{
	...
	private List<SessionCostTrackerPanel.GridItem> gridItems(List<long[]> items)
	{
		...
		if (items != null)
		{
			for (long[] it : items)
			{
				...
				out.add(new SessionCostTrackerPanel.GridItem((int) it[0], (int) it[1],
